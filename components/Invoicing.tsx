...
const InvoiceSinglePageView = ({ sale, config, onBack, notify }: { sale: SaleOrder, config: ERPConfig, onBack: () => void, notify: any }) => {
  ...
  const handleDownloadPDF = () => {
    ...
    if (!element) return;
    // Mise à jour vers format A6 pour le PDF
    const opt = { 
      margin: 0, 
  ...
  return (
    <div className="h-full flex flex-col space-y-6 animate-fadeIn pb-10">
      ...
      <div className="flex-1 flex justify-center overflow-y-auto scrollbar-hide py-4" dir={config.language === 'ar' ? 'rtl' : 'ltr'}>
        {/* Dimensions A6 forcées par CSS : 105mm x 148mm */}
          ...
          <div className="flex justify-between items-start border-b border-slate-200 pb-2 mb-2">
            <div className="flex items-start space-x-2 rtl:space-x-reverse">
              ...
              <div className="space-y-0">
                <h1 className="text-[10px] font-black uppercase text-slate-900 leading-tight">{config.companyName}</h1>
                <p className="text-[6px] font-bold text-slate-500 uppercase leading-tight">{config.address}</p>
              </div>
            </div>
            <div className="text-right rtl:text-left">
              <h2 className="text-[10px] font-black uppercase">{isRefund ? 'Avoir' : 'Facture'}</h2>
              <p className="text-[7px] font-mono font-black">#{sale.id.slice(-8)}</p>
              <p className="text-[6px] font-bold text-slate-400 mt-1">{new Intl.DateTimeFormat(locale, { dateStyle: 'short', timeZone: config.timezone }).format(new Date(sale.date))}</p>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-2 mb-3">
            <div className="p-1.5 bg-slate-50 rounded-lg border border-slate-100">
               <p className="text-[5px] font-black text-slate-400 uppercase mb-0.5">Client</p>
               <h3 className="text-[7px] font-black uppercase truncate">{sale.customer}</h3>
            </div>
            <div className="p-1.5 bg-slate-50 rounded-lg border border-slate-100 text-right rtl:text-left">
               <p className="text-[5px] font-black text-slate-400 uppercase mb-0.5">Mode</p>
            ...
            <table className="w-full text-[6.5px] border-collapse">
              <thead>
                <tr className="bg-slate-900 text-white">
                  <th className="px-1.5 py-1 text-left rtl:text-right uppercase">Art.</th>
          ...
          <div className="border-t border-slate-900 pt-2 mt-auto">
             <div className="flex justify-between items-end">
                <div className="flex items-center space-x-2 rtl:space-x-reverse">
                   {config.showQrCodeOnInvoice && <div className="p-0.5 border border-slate-900 rounded"><QrCode size={20} /></div>}
                   <p className="text-[5px] font-bold text-slate-400 max-w-[40mm]">TerraPOS+ v2.5. Système certifié.</p>
                </div>
                <div className="bg-slate-900 text-white p-2 rounded-lg shadow-sm min-w-[35mm] text-right rtl:text-left">
                   <p className="text-[5px] font-black text-purple-400 uppercase tracking-widest mb-0.5">TOTAL TTC</p>
